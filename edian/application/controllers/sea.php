...
class Sea extends MY_Controller {
     ...
     * @return 根据id得到的数据返回
     */
    protected function _sea($id,$currentPage) {
        /**********************/
        //将这个id保存到缓存中，将来
        $res = array();
        $timer = 0;
     ...
     */
    protected function _getIdArr($key) {
        $id = Array();
        if(is_array($key)){
            //如果传入的关键字数组
            $keyArrLen = count($key);
     ...
     */
    protected function _getSubItem($key) {
     ...
     * @return array 返回数据数组
     */
    protected function _getAnsBykey($key,$page) {
        /*
        ...
        foreach ($key as $idx => $val) {
            ...
            $tmp = $this->_sea($val,$page);//这里的$val必然是数组，id的数组
            if($tmp){
                $ans[$idx] = $tmp;
            }
        }
     ...
     */
    protected function _getAppend($idx) {
        ...
        if($cache){
            ...
            //通过_getIdArr 得到id，
            //$res[$idx] = $this->_getIdArr($idx);
            return $this->_getIdArr($idx);
        }
        return $res;
    }
    ...
    public function index() {
        ...
        if($keyword == "0"){
            ...
            if($app){
                ...
                //以非汉字，数字，字母为分界点开始分割;
                $key = $this->_getAppend($key[0]);//返回id数组
                $ans = $this->_sea($key,$currentPage);
            }else{
                $key = $this->_keyPreDel($keyword);
                //对关键字进行处理，得到id数组和value之后的排序，然后
                if(count($key) > 1){
                    $flag = 1;
                    foreach ($key as $idx => $val) {
                        //虽说为2维数组，但是第二纬只有一个数字，所以合并成为一维数组，然后unique
                        $tmp = $this->_sea($val,$currentPage);//这里的$val必然是数组，id的数组
                        if($tmp){
                            $ans[$idx] = $tmp;
                        }
                    }
                    ...
                    foreach ($key as $idx => $val) {
                        $ans = $this->_sea($val,$currentPage);//这里的key长度为1，所以$ans长度必然只能为1
                    }
                }
            }
            //$ans = $this->_getAnsBykey($key,$currentPage);
